...
/** Get median color from around the pixel */
Vec3b median(const Image& img, Point pos, int radius) {
    ...
    for (int dx = -radius; dx <= radius; dx++)
        for (int dy = -radius; dy <= radius; dy++) {
            ...
            if (x<0 || y<0 || x>=img.size().width || y>=img.size().height)
                continue;
            Vec3b color = img(pos.y + dy, pos.x + dx);
    ...
    int median = channel[0].size();
    if (median == 0)
        return Vec3b(0, 0, 0);
    median /= 2;
    
    return Vec3b(channel[0][median], channel[1][median], channel[2][median]);
}

/** Get color value corresponding to white.
 *
 * White point for a given image position is calculated from white points
 * from other positions as their average weighted by 1/(distance)^2.
 */
...
/** Multiplies color channels in a given rectangle by given multipliers */
...
/** Extracts white points from image */
...
/** Normalizes color and brightness */
void white_balance(Image& img, const WhitePoints& whitePoints) {
    ...
    for (int i=0; i<img.size().width; i+=TILE)
        for (int j=0; j<img.size().height; j+=TILE) {
            Vec3b color = get_white(whitePoints, Point(i, j));
            int gray = (color[0] + color[1] + color[2])/3;
            double buf[3];
            for (int k=0; k<3; k++){
                buf[k] = 255.0/((color[k]+gray)/2)*SHRINK_FACTOR;
            }
            balanceTile(img, Rect(i,j,TILE,TILE) & Rect(0, 0, img.size().width, img.size().height), buf);
        }
